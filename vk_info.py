...
def format_ai_info(
    *,
    ...
    lines.append("Обоснуй: reply на сообщение + «артем обоснуй» (или фото + «артем обоснуй»)")
    lines.append("Озвучка: «артем озвучь …» или «артем … озвучь»")
    lines.append("Ответить голосом: reply + «артем ответь ему»")
    lines.append("Спорить голосом: reply + «артем спорь»")
    ...
    lines.append("Цитата: reply + «/цит» (iOS) или «артем цитата» / «/цит петух|кибер|неон|минимал»")
    ...
    lines.append("Сменить промпт (владелец): /промпт … или «артем промпт …»")
    return "\n".join(lines)
